...
## L. Live-deployment findings (first real deploy, 2026-06-14)

Issues only a live multi-minute run on a real account could surface — static review and `--dry-run` could not. Found while verifying an end-to-end agent run on `philharmonic.vviswanathan.workers.dev`.

- [x] TL.1 **deploy gap** Cloudflare Access protects the whole hostname, blocking the agent's `/api/internal/*` run-token callbacks (302→login) so the agent can't read its task / post comments / update status. FIX (live): created a path-scoped Access app on `…/api/internal` with a Bypass policy — the Worker's run-token HMAC is the real auth there. README §20 Access steps must document this; SPEC §7.1/§16.1 should call it out. (left: add to README + SPEC)
- [x] TL.2 **bug (deployed code) — FIXED & confirmed live** `runAgent`'s long streaming `exec` doesn't renew the container activity timer → default `sleepAfter='10m'` fired mid-run → `WorkflowInternalError` → endless re-dispatch. FIX: `override sleepAfter = '150m'` on the Sandbox class (NB: SDK parser is `^\d+[smh]$` — `'2h30m'` is rejected as "invalid time expression"; my first attempt used that and broke prepare in a retry loop). Confirmed live: the run then stayed alive 30 min instead of dying at 10. Superseded as the blocker by TL.4.
- [ ] TL.4 **bug (deployed code) — BLOCKER, not yet fixed** With sleepAfter fixed, `runAgent` ran a full **30 minutes** then errored `WorkflowInternalError`: a single `sandbox.exec(cmd, {stream:true,onOutput})` held open through the Sandbox DO hits a ~30-min platform wall. FIX (per audit recommendation): rewrite runAgent to `startProcess(cmd)` + a polling loop of short `getProcessLogs` calls (short requests dodge the long-exec wall, renew activity, capture output incrementally), with `step.sleep` between polls; keep the run.log broadcast on each new chunk. `apps/worker/src/workflow/ImplementationRun.ts`.
- [x] TL.5 **RESOLVED — was a red herring** The "0 output" wasn't an Anthropic/egress problem: `claude` never *started*. It died instantly on `Failed to change directory to '/workspace/repo'` because the repo was never cloned (see TL.7). Fixed runAgent to `startProcess`+poll capturing **stderr** (which surfaced this), persist every poll, and a 2-min quiet-kill — that's what made the real error visible.
- [ ] TL.6 **bug (deployed code) — known, not yet fixed** `prepare` reuses the same sandbox (sandboxId == taskId) across a task's runs; after several manually-terminated runs the container wedges and `prepare`'s execs hang ~5 min → `WorkflowInternalError` → re-dispatch loop. Mitigation for testing: run a fresh task. Real fix: destroy the sandbox at the start of prepare (or per-run sandbox ids), and have manual/auto cancellation always call `sandbox.destroy()` (my D1-level cancels bypassed it).
- [x] TL.7 **§15 design gap — FIXED (code)** Private-repo `git clone` never sent a request (`could not read Username`). Fixed: placeholder credential in the clone URL (`x-access-token:egress-injected@`) + `GIT_TERMINAL_PROMPT=0` so git sends the request; egress injects real Basic auth.
- [x] TL.8 **egress registration bug — FIXED (the big one)** The class-level outbound handlers were registered on the `Sandbox` subclass, but the SDK's dispatch keys the static-handler registry by `props.className === "ContainerProxy"` — so they were NEVER found and NO egress injection happened for any host (this would also have broken Anthropic). Fixed by registering `OUTBOUND_HANDLERS` on `ContainerProxy` too. Verified live: `[egress] injectGitHubGit fired …` + token read (`len=93 prefix=gith`, real fine-grained PAT). Split github.com (git, Basic) from api.github.com (gh, Bearer). SPEC §15 must document the `ContainerProxy` registration requirement.
- [ ] TL.9 **BLOCKED ON USER — GitHub token** With egress now provably injecting the real 93-char `github_pat_` token (both `x-access-token:token` and token-as-username Basic forms tried), GitHub still returns `Invalid username or token. Password authentication is not supported for Git operations`. The injection is correct, so the fine-grained PAT itself isn't authenticating for git on `vaibhavviswanathan/philharmonic` — needs the user to verify Contents:Read on that repo / regenerate. Definitive test: `git clone https://<TOKEN>@github.com/vaibhavviswanathan/philharmonic` locally.
- [ ] TL.10 **bug (deployed code) — known** Repeated runs on a task wedge its sandbox (sandboxId==taskId, ephemeral fs, my D1-level cancels skipped `sandbox.destroy()`), so prepare hangs ~6min on a wedged container and re-dispatches. Fix per TL.6.
- [x] TL.3 **validated live** failure→auto-requeue self-healing works in production: runs whose Workflow errored were marked `failed`, tasks gated back to `ready` and re-dispatched, never stranded (the #1 audit fix — confirmed). Also confirmed live: Access enforcement + forged-JWT reject, SPA WebSocket, full transition→dispatch→claim chain, concurrency limit, sandbox boot + repo clone via GitHub egress injection, reconcile alarm not killing healthy runs.
